...
CHATUI_API = os.getenv("CHATUI_API_URL") # with or without cpu
#MODEL = "llama3:8b"
MODEL = "llama3.2:1b"
N_SAMPLES = 2
...
def estimate_uncertainty_from_samples(samples: list) -> np.ndarray:
    ...
    # Build a stats dictionary as required (wrap the matrix with np.array)
    stats = {
        "sample_texts": [samples],              # shape: (1, n)
        "semantic_matrix_entail": np.array([W])  # shape: (1, n, n)
    }
...
Cats have been human companions for thousands of years, with evidence suggesting that they were first domesticated in ancient Egypt around 4,000 years ago. Today, there are over 600 million domestic cats worldwide, making them one of the most popular pets globally!"""
    ...
    print("\nRaw uncertainty scores (one per input sample):")
    print(score_array)
...
def main(test=True):
    if test:
        ...
        if test_case == "new_similar":
            ...
            samples = [
                "Bananas are yellow and full of potassium.",
                "The Great Wall of China spans thousands of miles.",
                "Pythagoras developed a theorem that relates the sides of a right triangle.",
                "Mount Everest is the highest peak on Earth.",
                "Classical music often soothes the mind and body."
            ]
        else:
            raise ValueError("Invalid test_case value")
    else:
    ...
    score_array = estimate_uncertainty_from_samples(samples)
    print("\nRaw uncertainty scores (Eccentricity estimator):")
    print(score_array)
    final_score = float(score_array[0])
    print("\n=== Final Uncertainty Score for Test Case ===")
    print(final_score)
